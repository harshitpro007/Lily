...
export class EncryptionBaseDao {
  ...
  async updateOne(coll: any, searchObj: any, updateObj?: any, options?: {}) {
    try{
        updateObj['updated'] = Date.now();
        await coll.updateOne(searchObj, { $set: updateObj });
        return await coll.findOne(searchObj, options)
    }
    catch(error){
        return Promise.reject(error);
    }
  ...
  async update(coll: any, searchObj: any, updateObj?: any, options?: {}) {
    try{
        await coll.updateOne(searchObj, updateObj);
    }
    catch(error){
        return Promise.reject(error);
    }
  ...
  async distinct(coll: any, field: any, query = {}) {
    try{
        return await coll.distinct(
          field,
          query 
        );
    }
