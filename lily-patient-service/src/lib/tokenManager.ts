...
function readEpicPrivateKey(): Promise<string> {
	return promisify(readFile)(SERVER.EPIC_JWT_PRIVATE_KEY, "utf8");
}

function readEpicPublicKey(): Promise<string> {
	return promisify(readFile)(SERVER.EPIC_JWT_PUBLIC_KEY, "utf8");
}
...
const encodeEpicToken = async function(payload): Promise<string> {
	const cert = await readEpicPrivateKey();
	if (!cert) return Promise.reject("Error in token Generation");
	if (payload.exp) payload.exp = Math.floor((Date.now() + payload.exp) / 1000);
	return await Jwt.sign(payload, cert, { algorithm: SERVER.EPIC.EPIC_JWT_ALGO});
};
...
const createEpicToken = async function() {
	const currentTime = Math.floor(Date.now() / 1000)
	const accessToken = await encodeEpicToken({
		iss: SERVER.EPIC.EPIC_CLIENTID,
		aud: SERVER.EPIC.EPIC_TOKEN_LINK,
		sub: SERVER.EPIC.EPIC_CLIENTID,
		iat: currentTime,
		exp: SERVER.TOKEN_INFO.EXPIRATION_TIME.LINK_LIMIT,
		jti: SERVER.EPIC.EPIC_TOKEN_JTI
	});
	return accessToken;
};
